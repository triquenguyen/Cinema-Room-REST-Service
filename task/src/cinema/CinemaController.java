    ...
    public ResponseEntity<?> stats(@RequestParam(required = false) String password) {
        ...
        if (password != null && password.equals("super_secret")) {
            ...
            for (OrderedSeat orderedSeat : cinema.getOrderedSeats()) {
                income += orderedSeat.getTicket().getPrice();
            }

            statistic.put("current_income", income);
            statistic.put("number_of_available_seats", cinema.getAvailable_seats().size());
            statistic.put("number_of_purchased_tickets", cinema.getOrderedSeats().size());
